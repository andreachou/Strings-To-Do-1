...
function removeBlanks(inputText){
    var new_text= inputText.split(" ").join("");
...
function getDigits(inputText){
    // empty list to store numbers
    var numberArray = [];
    // itnerate the inputText character
    for (var i=0; i<inputText.length; i++){
        if (!isNaN(inputText[i])){  //NaN means Not-a-Number
            numberArray.push(inputText[i]);
        }
    }
    var num = numberArray.join("");
    console.log(num);
}
